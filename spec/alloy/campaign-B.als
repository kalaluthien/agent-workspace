/*
 * Campaign lifecycle -- variant B.
 *
 * Index mechanism: the anchor issue body holds an explicit checklist of member
 * issue URLs, maintained by hand.
 *
 * Modelled consequence: the index entry is a SECOND write, to a DIFFERENT
 * object, after the member issue already exists -- so it may simply not happen,
 * on the add path and on the remove path alike. Nothing else holds the truth,
...
sig CampaignB in Campaign { var checklist: set Issue }
fact { CampaignB = Campaign }

fun idx[c: Campaign]: set Issue { c.checklist }

pred idxFrame { checklist' = checklist }

/* Manual edit of the anchor body: it may or may not be made. */
pred addIdx[c: Campaign, i: Issue] {
  checklist in checklist' and checklist' in checklist + c->i
}
pred remIdx[c: Campaign, i: Issue] {
  checklist' in checklist and checklist - c->i in checklist'
}
pred initIdx { all c: Campaign | c.checklist = c.members }
