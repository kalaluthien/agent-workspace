/*
 * Campaign lifecycle -- variant C.
 *
 * Index mechanism: a per-campaign GitHub label (campaign:auth-refactor-260828)
 * applied to every member issue, queried with `gh search issues label:...`.
 *
 * Modelled consequences:
 *   - a label is a per-repository object, so it must be created in each member
 *     repository before an issue there can carry it; `gh issue create --label`
 *     fails loudly otherwise, so this is a guard, not a silent omission;
 *   - applying the label is part of creating the issue (one command);
 *   - REMOVING the label on removal is a second, manual edit and may be skipped.
 */
module campaignC
...
sig CampaignC in Campaign {
  var labelled:  set Issue,     -- issues currently carrying the campaign label
  var labelRepos: set Repo      -- repositories in which the label object exists
}
fact { CampaignC = Campaign }

one sig CreateLabel extends Event {}

fun idx[c: Campaign]: set Issue { c.labelled }

pred idxFrame { labelled' = labelled and labelRepos' = labelRepos }

/* Applied by the same command that creates the issue -- but only if the label
   object exists in that repository. */
pred addIdx[c: Campaign, i: Issue] {
  i.home in c.labelRepos
  labelled' = labelled + c->i
  labelRepos' = labelRepos
}
/* Un-labelling is a separate edit and may be forgotten. */
pred remIdx[c: Campaign, i: Issue] {
  labelled' in labelled and labelled - c->i in labelled'
  labelRepos' = labelRepos
}
pred initIdx {
  all c: Campaign | c.labelled = c.members and c.members.home in c.labelRepos
}

pred createLabel[c: Campaign, r: Repo] {
  r not in c.labelRepos
  labelRepos' = labelRepos + c->r
  labelled' = labelled
  Open' = Open and Merged' = Merged and pr' = pr and members' = members
  dirs' = dirs and st' = st
  Now.ev = CreateLabel and no Now.evIssue and no Now.evMachine
}

pred variantStep { some c: Campaign, r: Repo | createLabel[c, r] }
