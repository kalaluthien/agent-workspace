...
/* The issue and its index entry are one write. A member already in the index
   is never added again (`i not in Campaign.memberIssues`): a later discovery of
   the same mechanism reopens that issue, which is Open' = Open + i over an
   existing member and not this event (AGENTS.md § Sub-issues). Deliberately no
   session, machine or binding precondition: filing a sub-issue is a record, not a claim, so any
