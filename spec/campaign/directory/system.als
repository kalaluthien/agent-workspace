...
/* opening-campaign/scripts/acquire-repo.sh. On a re-run over an existing checkout it switches the
   ...
   guard alone otherwise. One writer owns a hook slot -- the installer, which adopts the guard's
   shim and refuses anything else -- because two writers left every delegate clone with no hook
   of the repository's own, and a commit there was never auto-pushed. */
