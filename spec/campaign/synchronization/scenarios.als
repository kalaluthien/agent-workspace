/*
 * The witnesses over synchronization/system: a container pull request landing under
...
pred S16b_ContainerBehindAfterMerge {
  one c: Campaign | some i: c.memberIssues {
    ...
    eventually Machine in ContainerBehind
    eventually Now.event = PullContainer
    eventually (no ContainerBehind and complete[i])
  }
}
...
pred S16c_BehindForever {
  one c: Campaign | some i: c.memberIssues {
    i.repo = Container
    always Now.event != PullContainer
    eventually (Now.event = MergePullRequest and Now.issue = i)
    eventually always Machine in ContainerBehind
  }
}
...
pred S16d_CloneFromUnpushedContainer {
  one c: Campaign | some m: Machine {
    ...
    eventually (m in ContainerUnpushed and Now.event = CreateDir and Where.machine = m)
    eventually (m in machinesHolding[c] and m in ContainerUnpushed)
  }
}
...
/* The superseded rule: never clone while the outer container holds commits
   origin lacks. */
pred pushBeforeClone { always (Now.event = CreateDir implies no ContainerUnpushed) }
...
pred cloneThenMergeThenLaunch[c: Campaign, i: Issue, m: Machine] {
  i in c.memberIssues and i.repo = Container
...
run S16b_ContainerBehindAfterMerge  for exactly 2 Issue, 1 PullRequest, exactly 1 Campaign, exactly 2 Machine, exactly 2 Repo, 1 Branch, 2 CampaignDir, 12 steps expect 1
...
run S16d_CloneFromUnpushedContainer for exactly 2 Issue, 1 PullRequest, exactly 1 Campaign, exactly 2 Machine, exactly 2 Repo, 1 Branch, 2 CampaignDir, 10 steps expect 1
