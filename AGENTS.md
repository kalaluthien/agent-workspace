...
with `campaign-claim take`: one ref, cut on the base.
**`scripts/check-campaign-claim.py` is what makes that true rather than
remembered** — a `PreToolUse` guard answering "is this a change to campaign work
that no claim of this session covers", refusing the ones that are. A change landing outside every base tree and
